...
DECLARE
  ...
  v_managed text[] := ARRAY[
    ...
    'pago_mensal','pago_anual','pago_anualvip','pago_grupovip_lotofacil','pago_aula_mega_especial',
    'gratis',
    'plano_vencido',
    'cancelado_ativo','cancelado_inativo',
    'grupovip_cancelado_ativo','grupovip_cancelado_inativo',
    'aula_mega_cancelado_ativo','aula_mega_cancelado_inativo',
...
BEGIN
  ...
  ELSIF v_plan_slug = 'aula-ao-vivo-mega-especial' THEN
    -- Produto WhatsApp puro (Aula Mega Especial 30 anos)
    IF NEW.status_assinatura = 'ativa'
       AND NEW.validade_assinatura IS NOT NULL
       AND NEW.validade_assinatura > v_now THEN
      v_tags := array_append(v_tags, 'pago_aula_mega_especial');
    ELSIF NEW.status_assinatura = 'cancelada' THEN
      IF NEW.validade_assinatura IS NOT NULL AND NEW.validade_assinatura > v_now THEN
        v_tags := array_append(v_tags, 'aula_mega_cancelado_ativo');
      ELSE
        v_tags := array_append(v_tags, 'aula_mega_cancelado_inativo');
      END IF;
    ELSE
      v_tags := array_append(v_tags, 'plano_vencido');
    END IF;
