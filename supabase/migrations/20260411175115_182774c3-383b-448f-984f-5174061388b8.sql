...
BEGIN
  ...
  -- Allow admins to make any change
  ...
  -- Permitir que o usuário mude o status de verificação de email APENAS de falso para verdadeiro
  IF NEW.email_verificado IS DISTINCT FROM OLD.email_verificado THEN
    IF NOT (OLD.email_verificado IS FALSE AND NEW.email_verificado IS TRUE) THEN
