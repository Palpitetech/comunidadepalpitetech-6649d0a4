...
BEGIN
    ...
    IF NOT v_exists AND p_celular IS NOT NULL AND p_celular <> '' THEN
        ...
        -- Assume-se que o banco armazena apenas números ou um formato padrão
        SELECT nome INTO v_nome 
        FROM public.perfis 
...
-- Permite acesso à função pelo papel anon
GRANT EXECUTE ON FUNCTION public.verificar_existencia_usuario(TEXT, TEXT) TO anon;
GRANT EXECUTE ON FUNCTION public.verificar_existencia_usuario(TEXT, TEXT) TO authenticated;
