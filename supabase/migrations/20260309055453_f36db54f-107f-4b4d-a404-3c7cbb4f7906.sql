...
BEGIN
  ...
  PERFORM extensions.http((
    ...
    ARRAY[
      ...
      extensions.http_header('x-webhook-secret', current_setting('app.notifications_webhook_secret', true))
    ],
    'application/json',
    jsonb_build_object(
      'tipo', 'novo_post',
...
CREATE TRIGGER trigger_push_on_new_post
  AFTER INSERT ON public.postagens
  FOR EACH ROW
  EXECUTE FUNCTION public.trigger_push_novo_post();
