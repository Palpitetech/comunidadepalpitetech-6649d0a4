// =============================================================================
// /gerador (Lotofácil) — wrapper sobre o pipeline determinístico shared.
// =============================================================================
...
serve(async (req) => {
  ...
  try {
    ...
    const qtdDezenas = clampQtdDezenas("lotofacil", Number(body.qtdDezenas));
    const periodoAnalise = Math.min(Math.max(Number(body.periodoAnalise) || 50, 1), 500);

    // Quota atômica (admin pula via maxPerDay = -1)
    ...
    const result = await gerarPalpitesDeterministicos({
      ...
      periodoAnalise,
      filtros: {
        dezenasFixas: body.dezenasFiexas, // mantém typo legacy do front
        dezenasExcluidas: body.dezenasExcluidas,
        pedidoEspecial: body.pedidoEspecial,
      },
      userId: auth.userId,
      supabaseAdmin: auth.supabaseAdmin,
      edgeFunction: "generate-palpites",
    });
    ...
    if (msg === "LOTERIA_NAO_SUPORTADA") return json(500, { error: "Loteria não suportada" });
    console.error("[generate-palpites] erro:", error);
