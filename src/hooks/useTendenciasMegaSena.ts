...
export interface TendenciaMegaSena {
  // Último resultado
  ultimoConcurso: {
    id: number;
    data: string;
...
export function useTendenciasMegaSena(periodo: number) {
  return useQuery({
    ...
    queryFn: async (): Promise<TendenciaMegaSena> => {
      ...
      // Último resultado - Mega Sena tem 6 dezenas
      const ultimoConcurso = {
        id: ultimo.concurso_id,
        data: ultimo.data_sorteio,
        dezenas: ultimo.dezenas,
        impares: 6 - (ultimo.qtd_pares ?? contarPares(ultimo.dezenas)),
      ...
      const filtros = {
        impares: {
          top3: calcularTop3Padrao(concursos, (c) => 6 - (c.qtd_pares ?? 0)),
      ...
      // Frequência de dezenas - Mega Sena tem 60 dezenas
      ...
      // Fixas: >= 20% frequência (ajustado para Mega Sena - 10% é a média)
      const fixas = dezenasAnalise
        .filter((d) => d.frequencia >= 20)
        .sort((a, b) => b.frequencia - a.frequencia);

      // Excluídas: <= 5% frequência
      ...
      // Grupos perfeitos
      const grupos = {
        dupla: calcularGrupos(concursos, 2),
