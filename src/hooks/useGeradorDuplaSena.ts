...
export interface JogoGeradoDuplaSena {
  dezenas: number[];
}

export interface GeradorDuplaSenaResult {
  jogos: JogoGeradoDuplaSena[];
...
export function useGeradorDuplaSena() {
  const [isLoading, setIsLoading] = useState(false);
  const [result, setResult] = useState<GeradorDuplaSenaResult | null>(null);
  ...
  const generatePalpites = async (
    ...
    filtros?: FiltrosGeradorDuplaSena
  ) => {
    setIsLoading(true);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-palpites-duplasena`,
        {
          method: "POST",
          headers: {
      ...
      toast({
        ...
        description: `${data.jogos.length} jogo(s) criado(s) para Dupla Sena.`,
      });
    } catch (err) {
