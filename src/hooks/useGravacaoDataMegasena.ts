...
export function useGravacaoDataMegasena() {
  return useQuery({
    queryKey: ["gravacao-megasena"],
    queryFn: async () => {
      ...
      // 2b. Frequência das dezenas nos últimos 12 concursos
      ...
      const frequenciaDezenas: FrequenciaDezena[] = Array.from(freqMap.entries())
        .map(([dez, freq]) => ({
          ...
          tipo: freq >= 4 ? "quente" as const : freq <= 1 ? "fria" as const : "neutra" as const,
        }))
      ...
      // 3. Call generate-palpites-megasena edge function
      const { data: sessionData } = await supabase.auth.getSession();
      if (!sessionData.session) throw new Error("Usuário não autenticado");

      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-palpites-megasena`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ quantidade: 3, qtdDezenas: 6, periodoAnalise: 50 }),
        }
      );
