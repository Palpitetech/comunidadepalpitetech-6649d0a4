...
export interface GeradorMegaSenaResult {
  jogos: JogoGerado[];
...
export function useGeradorMegaSena() {
  const [isLoading, setIsLoading] = useState(false);
  const [result, setResult] = useState<GeradorMegaSenaResult | null>(null);
  ...
  const generatePalpites = async (
    ...
    filtros?: FiltrosGeradorMegaSena
  ) => {
    setIsLoading(true);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      toast({
        ...
        description: `${data.jogos.length} jogo(s) criado(s) para Mega Sena.`,
      });
    } catch (err) {
