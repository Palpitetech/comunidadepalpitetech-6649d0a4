...
export default function ResultadosLotomania() {
  ...
  const { data: allResultados, isLoading, error } = useResultados("lotomania", 200);
  ...
  return (
    <MainLayout pageTitle="Resultados Lotomania">
      <div className="container-senior py-6">
        ...
        <ResultadoSheet open={!!selectedResultado} onClose={() => setSelectedResultado(null)} resultado={selectedResultado} loteria="lotomania" />
      </div>
    </MainLayout>
  );
}
    ...
    <button onClick={onClick} className="w-full py-3 px-3 text-left hover:bg-accent/20 transition-colors rounded cursor-pointer focus:outline-none focus-visible:ring-1 focus-visible:ring-primary/30">
      ...
      <div className="flex flex-wrap justify-start gap-1 mb-2">
        {resultado.dezenas.map((dezena: number, index: number) => (
          <div key={index} className={cn("w-[26px] h-[26px] rounded-full flex items-center justify-center", "text-[11px] font-semibold", "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300")}>
