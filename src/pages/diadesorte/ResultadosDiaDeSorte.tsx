...
export default function ResultadosDiaDeSorte() {
  ...
  const { data: allResultados, isLoading, error } = useResultados("diadesorte", 200);
  ...
  return (
    <MainLayout pageTitle="Resultados Dia de Sorte">
      <div className="container-senior py-6">
        {!isMobile && (
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-3">
              <BarChart3 className="h-7 w-7 text-green-600" />
              <h1 className="text-xl font-bold">🍀 Resultados Dia de Sorte</h1>
            </div>
            <SnapshotButton targetRef={contentRef} defaultTitle="Resultados Dia de Sorte" />
          </div>
        )}
        ...
        <ResultadoSheet open={!!selectedResultado} onClose={() => setSelectedResultado(null)} resultado={selectedResultado} loteria="diadesorte" />
      </div>
    </MainLayout>
  );
}
  ...
  return (
    <button onClick={onClick} className="w-full py-3 px-3 text-left hover:bg-accent/20 transition-colors rounded cursor-pointer focus:outline-none focus-visible:ring-1 focus-visible:ring-primary/30">
      <div className="text-left mb-2 flex items-center gap-2">
        ...
        {resultado.mes_sorte && (
          <Badge variant="secondary" className="text-[10px] px-1.5 py-0">📅 {resultado.mes_sorte}</Badge>
        )}
      </div>
      <div className="flex flex-wrap justify-start gap-1.5 mb-2">
        {resultado.dezenas.map((dezena: number, index: number) => (
          <div key={index} className={cn("w-[30px] h-[30px] rounded-full flex items-center justify-center", "text-sm font-semibold", "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300")}>
            {String(dezena).padStart(2, "0")}
          </div>
        ))}
      </div>
      <div className="text-left text-xs text-muted-foreground">{resultado.dezenas.length} dezenas sorteadas</div>
    </button>
  );
}
