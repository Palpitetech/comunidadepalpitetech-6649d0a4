...
export default function FrequenciaDecenasMegaSena() {
  const [periodo, setPeriodo] = useState<number>(50);
  const { data, isLoading, error } = useFrequenciaMegaSena(periodo);
  ...
  // Forte: >= média + 5%, Fraca: <= média - 5% (ajustado para Mega Sena)
  ...
  const getStatusColor = (status: "forte" | "fraca" | "neutra") => {
    switch (status) {
      case "forte":
        return "bg-megasena-primary text-white border-megasena-primary";
      case "fraca":
  ...
  const neutras = 60 - fortes - fracas;
      ...
      <div className="min-h-screen bg-background">
        <div className="px-4 py-6 space-y-4 max-w-3xl mx-auto">
          ...
          <div className="flex items-center justify-center gap-2">
            <Badge variant="outline" className="px-2 py-1 gap-1.5 bg-megasena-primary/10 border-megasena-primary/30 whitespace-nowrap">
              <TrendingUp className="h-3.5 w-3.5 text-megasena-primary" />
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="text-center py-2 px-3">
                              <span className={cn(
                                "inline-flex items-center justify-center w-7 h-7 rounded-md font-bold text-xs",
                                getStatusColor(status)
                              )}>
                            ...
                            <td className="py-2 px-3">
                              {status === "forte" && (
                                <Badge className="bg-megasena-primary text-white text-xs">Forte</Badge>
                              )}
                              {status === "fraca" && (
                                <Badge className="bg-red-500 text-white text-xs">Fraca</Badge>
                              )}
                              {status === "neutra" && (
                                <Badge variant="secondary" className="text-xs">Neutra</Badge>
                              )}
                            </td>
                          </tr>
                        );
            ...
            <CardContent className="py-2.5">
              <div className="flex items-center justify-center gap-4 text-xs">
                <div className="flex items-center gap-1.5 whitespace-nowrap">
                  <span className="w-4 h-4 rounded bg-megasena-primary"></span>
