...
import { useFrequenciaQuina } from "@/hooks/useFrequenciaQuina";
...
export default function FrequenciaQuina() {
  ...
  const { data: estatisticas, isLoading, error } = useFrequenciaQuina(periodo);
  ...
  return (
    <MainLayout pageTitle="Quentes e Frias – Quina">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-4">
        {!isMobile && (
          <div className="flex items-center justify-between gap-4">
            <h1 className="text-xl sm:text-2xl font-semibold text-foreground">
              Quentes e Frias – Quina
            </h1>
            <SnapshotButton
              targetRef={contentRef}
              defaultTitle={`Quentes e Frias Quina - Últimos ${periodo} concursos`}
            />
          </div>
        )}
