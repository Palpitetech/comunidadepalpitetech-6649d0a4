...
export default function ResultadosDuplaSena() {
  ...
  return (
    <MainLayout pageTitle="Resultados Dupla Sena">
      <div className="container-senior py-6">
        {!isMobile && (
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-3">
              <BarChart3 className="h-7 w-7 text-duplasena-primary" />
              <h1 className="text-xl font-bold">Resultados Dupla Sena</h1>
            </div>
            <SnapshotButton targetRef={contentRef} defaultTitle="Resultados Dupla Sena" />
          </div>
        )}
  ...
  return (
    <button onClick={onClick} className="w-full py-3 px-3 text-left hover:bg-accent/20 transition-colors rounded border-b cursor-pointer focus:outline-none focus-visible:ring-1 focus-visible:ring-primary/30">
      ...
      <div className="text-left text-xs text-muted-foreground mt-2">
        S1: P{resultado.qtd_pares}/M{resultado.qtd_moldura}/Σ{resultado.soma ?? "-"} | S2: P{resultado.qtd_pares_s2}/M{resultado.qtd_moldura_s2}/Σ{resultado.soma_s2 ?? "-"}
      </div>
    </button>
  );
}
