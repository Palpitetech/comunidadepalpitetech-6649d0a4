    ...
    <AdminLayout pageTitle="Carteira">
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div>
          ...
          <p className={`text-4xl font-bold ${saldo >= 0 ? "text-emerald-600" : "text-destructive"}`}>
            {formatCurrency(saldo)}
          </p>
          <div className="flex gap-6 mt-1">
            <span className="text-sm text-emerald-600 flex items-center gap-1">
              <ArrowUpCircle className="h-3.5 w-3.5" /> Entradas: {formatCurrency(totalEntradas)}
            </span>
            <span className="text-sm text-destructive flex items-center gap-1">
              <ArrowDownCircle className="h-3.5 w-3.5" /> Saídas: {formatCurrency(totalSaidas)}
            </span>
          </div>
        </div>
          ...
          <Card className="border-yellow-200 bg-yellow-50/30">
            <CardContent className="pt-4 space-y-3">
              <p className="text-sm font-medium">
                🏆 {boloesPendentes.length} bolão(ões) premiado(s) sem lançamento
              </p>
              {boloesPendentes.map((b: any) => (
                <div key={b.id} className="flex items-center justify-between text-sm">
                  <span>{b.codigo} · Concurso {b.concurso_numero}</span>
                  <Button size="sm" variant="outline" disabled={lancamentoLoading === b.id} onClick={() => lancarPremio(b)}>
                    {lancamentoLoading === b.id ? <Loader2 className="h-3 w-3 animate-spin" /> : "Lançar Prêmio"}
                  </Button>
                </div>
              ))}
            </CardContent>
                  ...
                  <div key={m.id} className="flex items-start justify-between py-2 border-b border-border/50 last:border-0">
                    <div className="space-y-0.5">
                      ...
                      <div className="flex items-center gap-2 ml-5">
                        ...
                        {m.bolao_codigo && <span className="text-xs text-muted-foreground">{m.bolao_codigo}</span>}
                        {m.referencia && <span className="text-xs text-muted-foreground">· {m.referencia}</span>}
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <span className={`text-sm font-semibold ${isEntrada ? "text-emerald-600" : "text-destructive"}`}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea placeholder="Ex: Prêmio Lotofácil concurso 3636" value={novoDescricao} onChange={(e) => setNovoDescricao(e.target.value)} />
            </div>
            <div>
              <Label>Referência (opcional)</Label>
              <Input placeholder="Ex: Concurso 3636" value={novoReferencia} onChange={(e) => setNovoReferencia(e.target.value)} />
            </div>
          </div>
          <DialogFooter>
