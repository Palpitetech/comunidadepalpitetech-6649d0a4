...
export default function AdminUsuarios() {
  ...
  const filteredIncludeSuggestions = allTags.filter(t => t.toLowerCase().includes(includeSearch.toLowerCase()) && !includeTags.includes(t));
  ...
  const tagFilterContent = (
    <div className="w-[calc(100vw-3rem)] max-w-80 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        {tagFilterActive && (
          <button onClick={clearTagFilters} className="text-[11px] text-primary hover:text-primary/80 font-medium">
            Limpar tudo
          </button>
        )}
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium">Contém a tag</span>
        </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {includeTags.map(tag => (
              <button
                ...
                className="group inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-medium bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 hover:border-emerald-500/40 transition-colors"
              >
                {tag}
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="h-8 text-xs pl-7 bg-muted/30 border-muted-foreground/10 focus:bg-background"
          />
        </div>
        {includeSearch && filteredIncludeSuggestions.length > 0 && (
          <div className="rounded-lg border border-border/50 bg-muted/20 p-1.5 max-h-28 overflow-auto space-y-0.5">
            {filteredIncludeSuggestions.map(tag => (
              <button
                key={`sug-inc-${tag}`}
                onClick={() => { toggleTag(tag, includeTags, setIncludeTags); setIncludeSearch(""); }}
                className="w-full text-left px-2.5 py-1.5 rounded-md text-xs hover:bg-muted/60 text-muted-foreground hover:text-foreground transition-colors"
              >
                {tag}
              </button>
            ))}
        ...
        {includeSearch && filteredIncludeSuggestions.length === 0 && (
          <p className="text-[11px] text-muted-foreground/60 italic pl-1">Nenhuma tag encontrada</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium">Não contém a tag</span>
        </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {excludeTags.map(tag => (
              <button
                ...
                className="group inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-medium bg-red-500/10 text-red-700 dark:text-red-400 border border-red-500/20 hover:border-red-500/40 transition-colors"
              >
                {tag}
                <X className="h-3 w-3 opacity-50 group-hover:opacity-100 transition-opacity" />
              </button>
        ...
        {excludeSearch && filteredExcludeSuggestions.length > 0 && (
          <div className="rounded-lg border border-border/50 bg-muted/20 p-1.5 max-h-28 overflow-auto space-y-0.5">
            {filteredExcludeSuggestions.map(tag => (
              <button
                key={`sug-exc-${tag}`}
      ...
      {/* Footer - exact match toggle */}
      <div className="border-t border-border/50 pt-3">
        <label className="flex items-center justify-between cursor-pointer">
          <span className="text-xs text-muted-foreground">Correspondência exata</span>
          <Switch checked={exactMatch} onCheckedChange={setExactMatch} />
        </label>
        <p className="text-[10px] text-muted-foreground/50 mt-1">
          {exactMatch ? "Usuário deve ter TODAS as tags selecionadas" : "Usuário deve ter QUALQUER tag selecionada"}
        </p>
      </div>
    </div>
  );
