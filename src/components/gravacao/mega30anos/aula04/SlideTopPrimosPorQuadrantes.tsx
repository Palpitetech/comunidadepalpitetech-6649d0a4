...
import { PRIMOS_QUADRANTE } from "./aula04Helpers";
...
export default function SlideTopPrimosPorQuadrantes({ concursos }: Props) {
  const dados = useMemo(() => {
    return Object.entries(PRIMOS_QUADRANTE).map(([q, dezenas]) => {
      const r = calcularEstudo(concursos, {
        estudoId: `aula04-quadrante-${q}`,
        agrupamento: "dezena",
        periodo: { tipo: "total" },
        topN: dezenas.length,
        restringirA: dezenas,
  ...
  return (
    <div className="w-full h-full flex flex-col pt-28 sm:pt-32 lg:pt-36 pb-3 sm:pb-4 px-2 sm:px-4">
      <Mega30Header
        aula={4}
        estudoNome="Top dezenas PRIMAS por QUADRANTE"
        tipoAnalise="Apenas primos de cada um dos 4 quadrantes — Top 2 destacados"
      />
