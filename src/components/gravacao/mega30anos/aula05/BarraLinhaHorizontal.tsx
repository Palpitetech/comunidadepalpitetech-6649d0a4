interface Props {
  ...
  max: number;
  destaque?: boolean;
  corDestaque?: string; // hex (default vermelho da casa)
}

export default function BarraLinhaHorizontal({
  linha,
  freq,
  pct,
  max,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-extrabold leading-none" style={{ fontSize: 30 }}>
          {linha}
        </span>
      </div>

      <div
        className="flex-1 h-9 rounded-md relative overflow-hidden"
        style={{
      ...
      >
        ...
        <span className="opacity-70 ml-2 text-base">×</span>
        <span className="opacity-80 ml-3 text-base">{pct.toFixed(1)}%</span>
      </div>
    </div>
  );
}
