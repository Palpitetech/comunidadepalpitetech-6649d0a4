...
import { PARES_COLUNA } from "./aula02Helpers";
...
export default function SlideTopParesPorColunas({ concursos }: Props) {
  const dados = useMemo(() => {
    return Object.entries(PARES_COLUNA).map(([coluna, dezenas]) => {
      const r = calcularEstudo(concursos, {
        estudoId: `aula02-coluna-${coluna}`,
        agrupamento: "dezena",
        periodo: { tipo: "total" },
        topN: dezenas.length,
        restringirA: dezenas,
  ...
  return (
    <div className="w-full h-full flex flex-col pt-24 pb-4 px-4">
      <Mega30Header
        aula={2}
        estudoNome="Top dezenas PARES por COLUNA"
        tipoAnalise="Apenas colunas pares (2,4,6,8,10) — Top 3 destacados"
      />
