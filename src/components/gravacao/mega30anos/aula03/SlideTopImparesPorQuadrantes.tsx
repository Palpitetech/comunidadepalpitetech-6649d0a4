...
import { IMPARES_QUADRANTE } from "./aula03Helpers";
...
export default function SlideTopImparesPorQuadrantes({ concursos }: Props) {
  const dados = useMemo(() => {
    return Object.entries(IMPARES_QUADRANTE).map(([q, dezenas]) => {
      const r = calcularEstudo(concursos, {
        estudoId: `aula03-quadrante-${q}`,
        agrupamento: "dezena",
        periodo: { tipo: "total" },
        topN: 6,
        restringirA: dezenas,
  ...
  return (
    <div className="w-full h-full flex flex-col pt-28 sm:pt-32 lg:pt-36 pb-3 sm:pb-4 px-2 sm:px-4">
      <Mega30Header
        aula={3}
        estudoNome="Top dezenas ÍMPARES por QUADRANTE"
        tipoAnalise="Apenas ímpares de cada um dos 4 quadrantes — Top 3 destacados"
      />
