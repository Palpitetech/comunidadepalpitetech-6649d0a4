...
import { IMPARES_LINHA } from "./aula03Helpers";
...
export default function SlideTopImparesPorLinhas({ concursos }: Props) {
  const dados = useMemo(() => {
    return Object.entries(IMPARES_LINHA).map(([linha, dezenas]) => {
      const r = calcularEstudo(concursos, {
        estudoId: `aula03-linha-${linha}`,
        agrupamento: "dezena",
        periodo: { tipo: "total" },
        topN: dezenas.length,
        restringirA: dezenas,
  ...
  return (
    <div className="w-full h-full flex flex-col pt-24 pb-4 px-4">
      <Mega30Header
        aula={3}
        estudoNome="Top dezenas ÍMPARES por LINHA"
        tipoAnalise="Top 3 ímpares destacados em cada uma das 6 linhas"
      />
