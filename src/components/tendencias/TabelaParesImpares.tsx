...
interface EstatisticaParesImpares {
  qtdPares: number;
  qtdImpares: number;
  ocorrencias: number;
  atrasoAtual: number;
...
type SortColumn = "ocorrencias" | "atrasoAtual" | "porcentagem" | "qtdPares";
type SortDirection = "asc" | "desc";

export function TabelaParesImpares() {
  ...
  const { data: estatisticas, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("resultados")
        .select("concurso_id, qtd_pares, qtd_impares")
      ...
      // Agrupar por combinação pares/ímpares
      ...
      data.forEach((r, index) => {
        const key = `${r.qtd_pares}-${r.qtd_impares}`;
        
        if (!agrupado.has(key)) {
          agrupado.set(key, {
            qtdPares: r.qtd_pares ?? 0,
            qtdImpares: r.qtd_impares ?? 0,
            ocorrencias: 0,
      ...
      const resultado: EstatisticaParesImpares[] = Array.from(agrupado.values())
        .map((item) => ({
          qtdPares: item.qtdPares,
          qtdImpares: item.qtdImpares,
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50 hover:bg-muted/50">
              <SortableHeader column="qtdPares">Qtd Pares</SortableHeader>
              <TableHead>Qtd Ímpares</TableHead>
          ...
          <TableBody>
            {sortedData.map((item) => (
              <TableRow 
                key={`${item.qtdPares}-${item.qtdImpares}`}
