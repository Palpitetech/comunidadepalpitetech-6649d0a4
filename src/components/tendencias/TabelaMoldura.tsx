...
type SortColumn = "ocorrencias" | "atrasoAtual" | "porcentagem" | "qtdMoldura";
type SortDirection = "asc" | "desc";

export function TabelaMoldura() {
  ...
  const { data: estatisticas, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("resultados")
        .select("concurso_id, qtd_moldura")
      ...
      // Agrupar por quantidade de moldura
      ...
      const resultado: EstatisticaMoldura[] = Array.from(agrupado.values())
        .map((item) => ({
          qtdMoldura: item.qtdMoldura,
          qtdMiolo: 15 - item.qtdMoldura, // Total de dezenas (15) - moldura = miolo
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50 hover:bg-muted/50">
              <SortableHeader column="qtdMoldura">Qtd Moldura</SortableHeader>
              <TableHead>Qtd Miolo</TableHead>
