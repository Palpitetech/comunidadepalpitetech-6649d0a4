...
import { getLinha, getColuna } from "@/lib/megasena";

interface Props {
  tipo: "linha" | "coluna";
  indice: number; // 1-6 para linhas, 1-10 para colunas
}
...
export function TabelaLinhaColunaMegaSena({ tipo, indice }: Props) {
  const { data: estatisticas, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Para linhas: max 6 dezenas (6 por sorteio, 10 posições por linha)
      // Para colunas: max 6 dezenas (6 por sorteio, 6 posições por coluna)
      const maxPossivel = 6;
            ...
            {estatisticas.map((item) => (
              ...
              >
                <TableCell className="px-2 py-2">
                  <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-megasena-primary/10 text-megasena-primary font-bold text-xs">
                    {item.qtdDezenas}
                  </span>
                </TableCell>
                <TableCell className="px-2 py-2">
