...
export function TabelaEstatisticaMegaSena({ config }: Props) {
    ...
    queryFn: async () => {
      ...
      const resultado: EstatisticaItem[] = Array.from(agrupado.values())
        .map((item) => ({
          ...
          valorComplementar: config.totalDezenas - item.valor,
              ...
              >
                ...
                <TableCell className="px-2 py-2">
                  <div className="flex items-center gap-1">
                    ...
                    <span className={cn(
                      "text-xs font-medium",
                      item.atrasoAtual >= item.mediaOcorrencia && "text-primary font-semibold"
                    )}>
                      {item.atrasoAtual}
                    </span>
                  </div>
