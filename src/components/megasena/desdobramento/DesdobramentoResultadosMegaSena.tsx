...
interface DesdobramentoResultadosMegaSenaProps {
  jogos: JogoGerado[];
  dezenasFixes?: number[];
...
export function DesdobramentoResultadosMegaSena({
  jogos,
  dezenasFixes = [],
  ...
  onVoltar,
  filtrosResumo,
}: DesdobramentoResultadosMegaSenaProps) {
  ...
  const palpitesFormatados = useMemo(() => 
    jogos.map((jogo, index) => ({
      id: `desdobramento-ms-${index}`,
      dezenas: jogo.dezenas,
      estrategia: "Desdobramento Mega Sena",
  ...
  const handleCriarPasta = async (nome: string, cor: string, loteria: string) => {
    try {
      ...
      setPastas(prev => [...prev, { id: data.id, nome: data.nome, cor: data.cor || "#22C55E" }]);
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container max-w-lg mx-auto px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              className="gap-1.5 bg-megasena-primary hover:bg-megasena-primary/90"
            >
                ...
                {filtrosResumo.impares && (
                  <Badge variant="outline" className="text-xs gap-1">
                    ...
                    {filtrosResumo.imparesEhPadrao && (
                      <span className="text-megasena-primary font-semibold">Padrão</span>
                    )}
                  </Badge>
                )}
        ...
        <PalpitesToolbar
          ...
          onCopiarSelecionados={() => {
            const selecionados = jogos.filter((_, i) => 
              toolbar.selected.has(`desdobramento-ms-${i}`)
            );
      ...
      <div className="container max-w-lg mx-auto px-4 pb-6 pt-3 space-y-3">
        {jogos.map((jogo, index) => {
          const id = `desdobramento-ms-${index}`;
          return (
            <JogoCardMegaSena
              key={id}
              index={index}
