...
interface ResultadosSheetQuinaProps {
  open: boolean;
...
export function ResultadosSheetQuina({
  open,
  onOpenChange,
  jogos: jogosIniciais,
  ...
  const handleCopiarTodos = async () => {
    ...
    toast({ title: "Todos copiados! 📋", description: `${jogos.length} palpite(s) copiado(s).` });
  };

  const handleCopiarSelecionados = async () => {
    if (selected.size === 0) {
      toast({ title: "Nenhum palpite selecionado", variant: "destructive" });
      return;
    }
    const indices = Array.from(selected).map(id => parseInt(id.replace('gerador-', ''))).sort((a, b) => a - b);
    const texto = indices.map((i) => formatJogoParaCopia(jogos[i], i)).join("\n");
    await navigator.clipboard.writeText(texto);
    toast({ title: "Copiado! 📋", description: `${selected.size} palpite(s) copiado(s).` });
  };
  ...
  const handleSelecionarSubpasta = async (pastaId: string) => {
    const palpitesParaSalvar = salvarTodosMode
      ? jogos
      : Array.from(selected).map(id => parseInt(id.replace('gerador-', ''))).sort((a, b) => a - b).map(i => jogos[i]);
    ...
    await salvarPalpites(palpitesParaSalvar, periodoAnalise, pastaId, getEstrategiaTexto(), estrategia, "quina");
  ...
  const handleExcluirSelecionados = () => {
    ...
    const indices = new Set(Array.from(selected).map(id => parseInt(id.replace('gerador-', ''))));
    ...
    toast({ title: "Excluído!", description: `${selected.size} palpite(s) removido(s).` });
    if (novosJogos.length === 0) { onClearAll(); onOpenChange(false); }
  };

  const handleExcluirTodos = () => { onClearAll(); onOpenChange(false); };
  ...
  const handleSelectAll = () => {
    if (selected.size === jogos.length) setSelected(new Set());
    else setSelected(new Set(jogos.map((_, i) => `gerador-${i}`)));
  };
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div className="sticky top-0 z-10 bg-background border-b">
          <SheetHeader className="px-4 py-3 flex-row items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => onOpenChange(false)} className="flex items-center gap-1 text-muted-foreground hover:text-foreground transition-colors">
                <ArrowLeft className="h-5 w-5" />
              </button>
              <SheetTitle className="text-lg font-bold">Palpites – Quina</SheetTitle>
        ...
        <div className="px-3 py-3 space-y-3">
          {estrategia && <EstrategiaCard estrategia={estrategia} />}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between py-4 border-t mt-4">
              <Button variant="outline" size="sm" onClick={() => setCurrentPage((p) => Math.max(0, p - 1))} disabled={currentPage === 0} className="h-10 px-4 gap-1">
              ...
              <Button variant="outline" size="sm" onClick={() => setCurrentPage((p) => Math.min(totalPages - 1, p + 1))} disabled={currentPage === totalPages - 1} className="h-10 px-4 gap-1">
