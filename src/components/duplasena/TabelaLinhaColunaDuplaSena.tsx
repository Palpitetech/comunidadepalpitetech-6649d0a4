...
import { getLinha, getColuna } from "@/lib/duplasena";

interface Props {
  tipo: "linha" | "coluna";
  indice: number; // 1-5 para linhas, 1-10 para colunas
  sorteio: "1" | "2";
}
...
export function TabelaLinhaColunaDuplaSena({ tipo, indice, sorteio }: Props) {
  const { data: estatisticas, isLoading } = useQuery({
    queryKey: [`duplasena-estatisticas-${tipo}-${indice}-${sorteio}`],
    queryFn: async () => {
      ...
      const data = (rawData || []).map((r: any) => ({ concurso_id: r.concurso, dezenas_sorteio1: r.dezenas, dezenas_sorteio2: r.dezenas_sorteio2 }));
      ...
      // Para Dupla Sena: max 6 dezenas por sorteio
      const maxPossivel = 6;
      ...
      // Inicializar com 0-6 (possíveis quantidades em 6 dezenas sorteadas)
      for (let i = 0; i <= maxPossivel; i++) {
        agrupado.set(i, { qtd: i, ocorrencias: 0, ultimaOcorrencia: 0 });
      }

      data.forEach((r) => {
        const dezenas = sorteio === "1" 
          ? (r.dezenas_sorteio1 as number[]) 
          : (r.dezenas_sorteio2 as number[]);
  ...
  const getOcorrenciaBadgeColor = (ranking: number) => {
    if (ranking <= 1) return "bg-emerald-500 text-white";
    if (ranking <= 2) return "bg-amber-500 text-white";
            ...
            {estatisticas.map((item) => (
              ...
              >
                <TableCell className="px-2 py-2">
                  <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-duplasena-primary/10 text-duplasena-primary font-bold text-xs">
                    {item.qtdDezenas}
                  </span>
                </TableCell>
                <TableCell className="px-2 py-2">
                ...
                <TableCell className="px-2 py-2">
                  <div className="flex items-center gap-1">
                    ...
                    <span
                      className={cn(
                        "text-xs font-medium",
                        item.atrasoAtual >= item.mediaOcorrencia && "text-emerald-600"
                      )}
                    >
                      {item.atrasoAtual}
                    </span>
